...
@app.route('/course2.html', methods=['GET', 'POST'])
def course2():
    return render_template("course2.html")
@app.route('/course3.html', methods=['GET', 'POST'])
def course3():
...
def loggedin():
    if 'username' in session and session['user_type'] == 'student':
        username = session['username']
        student = get_student_instance(username)
        return render_template('loggedin.html', username=username,student=student)
    else:
...
def editstudent_form():
    ...
    if request.method == 'POST':
        ...
        user = get_student_instance(username)
        course = get_course(username)
        return render_template('studentprofile.html', student=user,course=course)
    else:
        cursor = conn.cursor()
        # execute SQL query to select interests from the STUDENT table
        cursor.execute('SELECT subjects FROM TUTOR')
